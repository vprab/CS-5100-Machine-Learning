    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                counter[p] = emissionModel[dist] * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                new_beliefs[newPos] += self.beliefs[p]*newPosDist[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [legal_pos[i % len(legal_pos)] for i in range(num_particles)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                dist = util.manhattanDistance(pacmanPosition, p)
                counter[p] += emissionModel[dist]

        self.beliefs = counter

        if self.beliefs.totalCount() is 0:
            self.initializeUniformly(gameState)
        else:
            self.beliefs.normalize()
            self.particles = [util.sample(self.beliefs) for i in range(len(self.particles))]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [permutations[i % len(permutations)] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        jailedGhosts = [i for i in range(self.numGhosts) if noisyDistances[i] is None]

        counter = util.Counter()

        for p in self.particles:
            for g in jailedGhosts:
                p = self.getParticleWithGhostInJail(p, g)

            prob = 1
            for i in range(self.numGhosts):
                if i not in jailedGhosts:
