...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        if self.q_vals.get(state):
            return self.q_vals.get(state).get(action) or 0.0
        else:
            return 0.0
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        new_q_val = (1 - self.alpha)*self.getQValue(state, action) + self.alpha*(reward + self.discount*self.getValue(nextState))
        if not self.q_vals.get(state):
            self.q_vals[state] = {}

        if not self.q_vals[state].get(action):
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for key in self.featExtractor.getFeatures(state, action).keys():
            q_val += self.weights[key] * self.featExtractor.getFeatures(state, action)[key]
